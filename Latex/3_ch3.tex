...
The sensor used to collect data for this investigation was the Livox Avia. The Livox Avia is a solid-state laser and Risley prism-based \ac{LiDAR} scanner with a built-in inertial measurement unit (\ac{IMU}). The sensor makes use of a 905 nm near infrared laser *edit here*
...
Return intensity stuff here. This information is stored as an 8-bit value (0-255) associated to each point.
...
\ac{FOV}                         & \begin{tabular}[c]{@{}l@{}} Non-repetitive Scanning Pattern: 70.4° (H) × 77.2 (V)\\ Repetitive Scanning Pattern: 70.4° (H) × 4.5° (V)\end{tabular}          &  &  &  \\ \cline{1-2}
...
\ac{IMU}                         & Bosch Sensortec BMI088                     &  &  &  \\ \cline{1-2}
...
\ac{LiDAR} is used over other remote sensing methods for various reasons. Being an active sensing method is advantageous over visual method such as telephoto or stereoscopic cameras as it does not require external illumination to observe the target. Additionally, with the correct sensor selection, \ac{LiDAR} is less susceptible to issues such as lens flare or overexposure to which typical cameras are prone. Certain \ac{LiDAR} sensors also have the ability to penetrate vegetation, allowing mapping of ground surfaces beneath areas obscured by plant cover.\\
Compared to radar, \ac{LiDAR} typically has superior mapping precision and target resolution, allowing more accurate reproductions of the target and the surrounding environment.
Using \ac{LiDAR} is not without its disadvantages. \ac{LiDAR} sensors are more expensive than more traditional remote sensing hardware. Unlike cameras, which can recover details such as colour, \ac{LiDAR} is only able to recover the three-dimensional geometry of the target. Radar outperforms \ac{LiDAR} in certain applications. It is capable of much larger detection ranges than \ac{LiDAR} and is also unaffected by adverse environmental conditions such as smoke, rain, and fog.

especially \ac{LiDAR} which is typically used in urban environments or for terrain mapping.
...
The Livox Avia has a listed beam divergence of 0.28° x 0.03° along its vertical and horizontal plane. This results in the emitted beam, if falling on a target perfectly perpendicular to the axis of propagation at a distance of 50 meters, having the form of an ellipse with the approximate dimensions of 244.35 x 26.18 mm. *insert fig here* 
