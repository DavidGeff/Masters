...
Many common point cloud processing methods are built around the assumption that the point cloud being worked upon is in the format of a standard (MxN) grid layout. Methods such as point cloud registration (combining multiple point clouds to form a single scene), clustering and model fitting often use the assumption of a constant grid to achieve their outputs.

%====================================================
\section{Clustering Algorithms}
\label{sec:ch3:section5}

Something about clustering algortithms. Suited for use depeding on properties of the data to be clustered (properties such as data dimensions, number of datapoints and the relationsip between individual datapoints), knowledge of the final classes the data will be clustered into and computational restraints. Clustering Algorithms can be categorised based on the methods by wich they function, some of the most common categories are Partitional, Heirachy, Density, Model-based and Spectral-based.