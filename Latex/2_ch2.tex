...
The Marginal Ice Zone (MIZ) is widely accepted to be the transitional region where the open ocean transitions to consolidated ice cover. This region is characterized by constant interactions between ice above and below the ocean surface, the atmosphere and ocean. There is no universally accepted definition of the MIZ, which leads to difficulties in understanding and comparing studies on the area by different authors who have varying definitions of the MIZ. \cite{vichiIndicatorSeaIce2022} states the MIZ is generally seen as the region where sea ice concentration (SIC) is between 15\% and 80\%, but this simple threshold based definition has proved to be a less reliable for the Antarctic, where ice drift and wave activity has been reported in areas classified as having 100\% ice cover. 

Due to its location, the MIZ is a highly dynamic region, leading to greater variability in its conditions, processes and dynamics when compared to other ice-covered regions. Various different types of ice make up the MIZ, but the correlation between SIC and ice type (frazil, pancake, grease, etc.) have not been fully characterized (\cite{vichiIndicatorSeaIce2022}).

*refine and add a bit here*

During the summer months the MIZ predominantly consists of large floes, capable of reaching sizes in the hundreds of meters, formed by the seasonal pack ice breakup. This is contrasted by the winter months, when the MIZ is dominated by a huge number of smaller pancake ice floes (\cite{alberelloPhysicalModelWave2021}).
...
%====================================================
\section{Ocean Waves in the MIZ}
\label{sec:ch2.section3}
%----------------------------------------------------

In the Antarctic MIZ wave activity is predominantly driven by wind activity and atmospheric pressure, resulting in wave energy most commonly occurring at frequencies below 0.1 Hz (\cite{thomsonWavePropagationMarginal2022}). These waves have been recorded propagating over 100 km into the MIZ, propagation to this extent is a rare occurrence in the Arctic. These waves are classified as gravity waves (\cite{holthuijsenWavesOceanicCoastal2007}), as the primary force attempting to return the disturbance to equilibrium is gravity.
...
Remote sensing has been the most common method for gathering data on the Antarctic region ever since satellite observation became widespread and the standard method for Earth observation and data collection. The methods for data collection has typically taken the form of Synthetic Aperture Radar (SAR) imaging and Optical imaging (\cite{maslanikRemoteSensingAntarctica1990}). The use of lasers and LIDAR in remote sensing is a recent development that has seen limited implementation in the past two decades (\cite{pettorelliSatelliteRemoteSensing2014}).
...
The first use of SAR to collect data over the Antarctic used the RADARSAT-1 satellite through the RADARSAT-1 Antarctic Mapping Project (RAMP), with the first phase of the project being completed in October 1997 (\cite{jezekRADARSAT1AntarcticMapping2002}).
Laser altimetry was first implemented aboard the Ice, Cloud and land Elevation Satellite (ICESat), through the Geoscience Laser Altimeter System (GLAS). ICESat was a dedicated polar observation satellite, with the primary objective of monitoring the polar ice-sheets. The GLAS produced elevation and atmospheric profiles and was launched in 2003, with data collection commencing on February of that year (\cite{schutzOverviewICESatMission2005}).
After the success of ICESat, ICESat-2 was launched in September 2018, with the Advanced Topographic Laser Altimeter System (ATLAS) aboard the satellite. The ATLAS sensor marked the first time a LIDAR sensor was equipped to an observation satellite (\cite{palmICESat2AtmosphericChannel2021}).
...
These gaps in our understanding of the MIZ is a clear indication that further investigation into the dynamics of the region is necessary.
