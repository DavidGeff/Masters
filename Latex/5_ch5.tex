...
\ac{RANSAC} has been applied with great success in identifying and reconstructing buildings in urban environments from airborne \ac{LiDAR} data (\cite{forlaniBuildingReconstructionVisualization2003}), but has not seen much use in natural environments. As \ac{RANSAC} is designed to fit a user-defined model to data, with a sufficiently complex model it may be possible to fit environmental features found in the \ac{MIZ}, such as surface waves propagating through an ice field. This would require a suitable wave model to be developed, but if successful could allow for the extraction of wave parameters such as amplitude and wave period directly from point cloud data.

For the data collected at Aalto University, a simple sinusoidal wave model may be sufficient to capture the wave characteristics, as the waves were generated using a wave maker and were relatively uniform and constrained to limit complexity. However, for data collected from the \ac{SCALE} Cruise, a more complex wave model will be required. This is due to increased dynamics and irregular nature of waves in the \ac{MIZ}, which are driven by various environmental factors and may also interact with the vessel itself. A Stokes II wave model may be suitable for this application.
